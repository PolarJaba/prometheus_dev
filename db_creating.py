...
class DatabaseManager:
    ...
    def create_raw_dictionaries(self):
        try:
            ...
            """
            self.cur.executescript(create_raw_dict_table_query)
            self.conn.commit()
    ...
    def create_dictionaries(self):    
        try:
            ...
            """
            self.cur.executescript(create_dict_table_query)
            self.conn.commit()
    ...
    def create_not_actual_link_tables(self):
        try:
            ...
            """   
            self.cur.executescript(create_not_actual_link_tables_query)
            self.conn.commit()
    ...
    def create_link_tables(self):
        try:
            ...
            """   
            self.cur.executescript(create_link_tables_query)
            self.conn.commit()
...
class DataManager:
    ...
    #     # Пусть данные модели (изменение таблиц словарей) переданы как dict
    #     self.from_model_dict = {'job_formats': {'id': [1, 2, 3], 'job_format_titles': ['DE', 'DS', 'DA']}, 'job_types', 
    #                             'specialities',
    #                             'skills', 'towns'}
    ...
    def load_without_changes(self, df_name):
        ...
        df = globals()[df_name]
        if not df.empty:
            df.to_sql(df_name, conn, schema=self.schema, if_exists='replace', index=False)
        else:
    ...
    def load_dictionaries_core(self, df_name):
        ...
        df = globals()[df_name]
        if not df.empty:
            selected_columns = df['id', 'title']
            selected_columns.to_sql(df_name, conn, schema=self.schema, if_exists='replace', index=False)
        else:
            self.log.info(f'Нет данных для загрузки, фрейм {df_name} пуст')


    # Функция первичной (и загрузки дельты) загрузки и обновления словарей
    def load_data_to_dicts(self):
        ...
        for dict_table_name in self.dictionary_tables_lst:
            try:
                ...
                self.load_dictionaries_core(str(f'{dict_table_name}').replace('raw_', ''))
                self.conn.commit()
            except Exception as e:
    ...
    def load_data_to_vacancies(self):
        if not self.vacancies.empty:
            try:
                self.vacancies.to_sql('vacancies', conn, schema=self.schema, if_exists='replace', index=False)
                self.conn.commit()
                self.log.info('Данные успешно загружены в таблицу vacancies')
            except Exception as e:
                self.log.error(f'Ошибка при зашрузке данных в таблицу vacancies: {e}')
    ...
    def load_data_to_links(self):
        for link_table_name in self.link_tables_lst:
            try:
                # Загрузка данных на core
                self.load_without_changes(link_table_name)
                self.conn.commit()
            except Exception as e:
    ...
    def delete_not_actual_core_data(self):
        ...
        # Здесь нужно обновить все таблицы 
        try:
            self.cur.executemany(load_not_actual_data_to_na_vacancy, tuple(not_actual_df['url']))
            self.cur.executemany(delete_not_actual_data_query, tuple(not_actual_df['url']))
            # self.cur.execute(load_not_actual_data_to_na_vacancy, (tuple(not_actual_df['url']),))
            # self.cur.execute(delete_not_actual_data_query, (tuple(not_actual_df['url']),))
            ...
            for table_name in self.link_tables_lst:
                ...
                """
                delete_data_query = f"""
                DELETE FROM {self.schema}.{table_name} WHERE id IN %s
                """
                self.cur.executemany(move_data_query, tuple(not_actual_df['id']))
                self.cur.executemany(delete_data_query, tuple(not_actual_df['id']))
                # self.cur.execute(move_data_query, (tuple(not_actual_df['id']),))
                # self.cur.execute(delete_data_query, (tuple(not_actual_df['id']),))
            self.cur.commit()
        except Exception as e:
...
# Создаем DAG ручного запуска (ddl)
...
hello_bash_task >> init_golden_core >> end_task
hello_bash_task >> move_data_from_golden_core >> end_task
hello_bash_task >> update_golden_core >> end_task
